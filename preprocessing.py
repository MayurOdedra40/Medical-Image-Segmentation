...
print(f"Mask labels still intact: {np.unique(p['ed_mask'])}")  # should be [0 1 2 3]
...
print(f"Image batch: {images.shape}  dtype: {images.dtype}")  # (8, 1, 256, 256)
print(f"Mask batch:  {masks.shape}   dtype: {masks.dtype}")   # (8, 256, 256)
print(f"Mask labels: {masks.unique()}")                        # tensor([0, 1, 2, 3])
